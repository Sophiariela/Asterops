...
export function CaseStudyCta({ caseStudy }: { caseStudy: CaseStudy }) {
  const { title, description, primaryCta } = caseStudy.cta;
  ...
  return (
    <section className="border-t border-border/60 px-6 py-24">
      <Reveal className="mx-auto flex max-w-3xl flex-col items-center gap-6 rounded-xl border border-border bg-card px-8 py-16 text-center">
        <h2 className="text-balance text-3xl font-medium tracking-tight text-foreground sm:text-4xl">{title}</h2>
